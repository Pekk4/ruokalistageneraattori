...
class TestMealRepository(unittest.TestCase):
    def setUp(self):
        ...
        self.meal_parameters = {"meal":"Surströmming och pepparkakor", "user_id":1}
    ...
    def test_find_all_meals_calls_read_method_correct(self, read_mock):
        self.repository.find_all_meals(1)

        read_mock.assert_called_with("SELECT id, name FROM meals WHERE user_id = :user_id", {"user_id":1})
    ...
    def test_find_all_meals_returns_correct_when_no_results(self, read_mock):
        ...
        result = self.repository.find_all_meals(1)
    ...
    def test_insert_meal_calls_write_method_correct(self, write_mock):
        ...
        self.repository.insert_meal(self.meal_parameters["meal"], 1)
    ...
    def test_insert_meal_returns_correct_when_no_errors_occurred(self, write_mock):
        ...
        self.assertEqual(self.repository.insert_meal("Kinuskikakku", 1), 666)
    ...
    def test_insert_meal_raises_exception_when_errors_occurred(self, write_mock):
        ...
        with self.assertRaises(InsertingError) as error:
            self.repository.insert_meal("Hapansilakkavoileivät", 1)
    ...
    def test_insert_ingredients_calls_write_many_method_correct(self, write_mock):
        ...
        parameters = [{"ingredient":ingredient, "user_id":1} for ingredient in self.ingredients]

        self.repository.insert_ingredients(self.ingredients, 1)
    ...
    def test_insert_ingredients_returns_correct_when_no_errors_occurred(self, write_mock):
        ...
        return_value = self.repository.insert_ingredients(self.ingredients, 1)
